...
test('config defaults optimized IP to disabled', () => {
  ...
  assert.equal(config.节点参数.优选IP.启用, false);
...
test('config accepts optimized IP settings', () => {
  const config = normalizeGlobalConfig({
    节点参数: {
      优选IP: { 启用: true, 随机端口: false, 自定义IP源: '104.16.0.1' },
    },
  });
  assert.equal(config.节点参数.优选IP.启用, true);
  assert.equal(config.节点参数.优选IP.随机端口, false);
  assert.equal(config.节点参数.优选IP.自定义IP源, '104.16.0.1');
});
